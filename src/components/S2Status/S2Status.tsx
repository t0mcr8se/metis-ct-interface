...
import {
  ...
  useSpinPrice,
  useSpins,
} from "../../hooks/useSpinningContract";
...
function S2Status() {
  ...
  const updateSpins = (e: BaseSyntheticEvent) => {
    ...
    if (isNaN(inp)) return setInput(0);
    setInput(inp);
  };
  ...
  useEffect(() => {
    if (isError) {
      setOpenedModal(true);
      if (error) setContent(error.name + error.message + error.stack);
    ...
    <>
      <div className={styles.card_container}>
        <img
          ...
          loading="lazy"
          src="https://cdn.builder.io/api/v1/image/assets/TEMP/4960abef5065742eaa234a9c64dc62d5092bc5869a0fb0d4a95ccfcc85efe779?apiKey=b68aa9ae8d4a4bd695329e075295d389&"
        />
        <div className={styles.card_info}>
          {Number(spins || 0)} Spins &nbsp;
        </div>
        <input
          className={`${styles.add_spins_input} ${disabled}`}
          value={numSpins}
          onChange={updateSpins}
        />
        <button
          className={`${styles.card_info} ${disabled}`}
          onClick={buySpins}
        >
          &nbsp; Buy Spins &nbsp;
        </button>
      ...
      {!disabled && (
        <div className={styles.card_container}>
          <div className={styles.card_info_2}>
            Cost: {formatEther(spinPrice ?? 0n)} METIS
          </div>
        </div>
      )}
      <TxModal
